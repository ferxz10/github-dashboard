...
export default function Dashboard({ username, onBack }) {
  ...
  const fetchData = async () => {
    ...
    setError('')
    try {
      const res = await fetch(`${API_URL}/api/dashboard/${username}`)
      if (!res.ok) {
    ...
    <div style={styles.page}>
      ...
      <main style={styles.main}>
          ...
          <div style={styles.profileCard}>
            ...
            <div style={styles.profileInfo}>
              ...
              <div style={styles.profileMeta}>
                ...
                {blogUrl && (
                  <a href={blogUrl} target="_blank" rel="noopener noreferrer" style={styles.metaLink}>
        ...
        <section style={styles.statsGrid} className="fade-in-1">
          ...
          <StatCard label="Own Repos"     value={stats.original_repos}    color="#00d084" icon="💡" />
        </section>
        ...
        <footer style={styles.footer}>
          <span>Data from the GitHub REST API · No API key required</span>
          <span>Built with FastAPI + React + Chart.js</span>
        </footer>
      </main>
    </div>
  )
    ...
    <div style={styles.page}>
      ...
      <main style={styles.main}>
        <div style={styles.loadingState}>
          ...
          <p style={styles.loadingSubText}>Loading repositories, activity and languages</p>
      ...
      <main style={{ ...styles.main, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={styles.errorCard}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button onClick={onBack}  style={styles.errorBtnSecondary}>← Back</button>
...
const styles = {
  ...
  headerTitle: {
    ...
    fontSize: '0.875rem', color: '#8b949e', letterSpacing: '0.05em',
  },
  backBtn: {
  ...
  refreshBtn: {
    ...
    background: 'transparent', border: '1px solid #21262d',
    color: '#8b949e', padding: '6px', borderRadius: '8px', cursor: 'pointer',
  },
  main: {
  ...
  avatar: {
    ...
    border: '2px solid #30363d', flexShrink: 0,
  },
  profileInfo: { flex: 1 },
  profileName: { fontSize: '1.6rem', fontWeight: 700, color: '#e6edf3', marginBottom: '2px' },
  profileLogin: { fontFamily: "'JetBrains Mono', monospace", fontSize: '0.9rem', color: '#00d084', marginBottom: '10px' },
  profileBio: { color: '#8b949e', fontSize: '0.95rem', marginBottom: '14px', lineHeight: 1.6, maxWidth: '600px' },
  profileMeta: { display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'center' },
  metaItem: { display: 'flex', alignItems: 'center', gap: '5px', color: '#8b949e', fontSize: '0.82rem' },
  metaLink: { display: 'flex', alignItems: 'center', gap: '5px', color: '#58a6ff', fontSize: '0.82rem', textDecoration: 'none' },
  ...
  chartCard: { background: '#161b22', border: '1px solid #21262d', borderRadius: '12px', padding: '24px', minWidth: '280px' },
  sectionTitle: {
  ...
  reposGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '14px' },
  footer: {
    display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap',
    gap: '8px', paddingTop: '16px', borderTop: '1px solid #21262d',
    color: '#484f58', fontSize: '0.78rem',
  },
  loadingState: { textAlign: 'center', padding: '80px 24px' },
  loadingIcon: { fontSize: '3rem', marginBottom: '16px' },
  loadingText: { color: '#e6edf3', fontSize: '1.1rem', fontWeight: 600, marginBottom: '6px', fontFamily: "'JetBrains Mono', monospace" },
  loadingSubText: { color: '#484f58', fontSize: '0.85rem', marginBottom: '28px' },
  progressBar: { width: '200px', height: '3px', background: '#21262d', borderRadius: '2px', margin: '0 auto', overflow: 'hidden' },
  progressFill: { height: '100%', background: '#00d084', borderRadius: '2px', animation: 'shimmer 1.5s infinite', backgroundSize: '200% 100%', backgroundImage: 'linear-gradient(90deg, #0a0c10 0%, #00d084 50%, #0a0c10 100%)' },
  errorCard: { background: '#161b22', border: '1px solid #30363d', borderRadius: '12px', padding: '40px', textAlign: 'center', maxWidth: '400px' },
  errorBtnPrimary: { background: '#00d084', color: '#0a0c10', border: 'none', padding: '10px 20px', borderRadius: '8px', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer' },
  errorBtnSecondary: { background: 'transparent', color: '#8b949e', border: '1px solid #30363d', padding: '10px 20px', borderRadius: '8px', fontSize: '0.875rem', cursor: 'pointer' },
}